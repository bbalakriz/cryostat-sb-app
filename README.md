...
1. This demo shows a simple REST service that returns a simple message.
   To troubleshoot the application start it with Flight Recorder.

   java -XX:StartFlightRecording -jar target\bala.app-1.0.0.jar
...
2. Open a web brower to http://localhost:8080/no-issues
   It will display some JSON with an id that is
   increased with every reload.

   Also open the following addresses:

   http://localhost:8080/gc-intensive and notice it is slower than http://localhost:8080/no-issues
   http://localhost:8080/cpu-intensive and notice it is slower than http://localhost:8080/no-issues
   http://localhost:8080/lock-contention and notice it is slower than http://localhost:8080/no-issues
...
4. Start a second shell where JDK_14\bin is on the path. To dump a recording file,
   ...
   $ jcmd bala.app.Application JFR.dump filename=dump.jfr
...
6. Switch to the web browser and reload http://localhost:8080/no-issues a few times.
   ...
   - Open http://localhost:8080/cpu-intensive
     It should print the stack trace of the methods that was running
     at the same time as the request

   - Open http://localhost:8080/gc-intensive (may need to reload to trigger GC)
     It should print the GCs pauses that happened when the request was running

   - Open http://localhost:8080/lock-contention
